...
Generate unique UUIDs with `uuidgen`. Customize the settings to match your organizationâs requirements.
...
You can also configure [local models](models.md). [Learn more](models.md).
...
Bearer tokens (`AWS_BEARER_TOKEN_BEDROCK` or `/connect`) take precedence over profile-based authentication. See [authentication precedence](providers.md) for details.

[Learn more about Amazon Bedrock configuration](providers.md).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](agents.md) youâve defined. If the specified agent doesnât exist or is a subagent, OpenCode will fall back to `"build"` with a warning.
